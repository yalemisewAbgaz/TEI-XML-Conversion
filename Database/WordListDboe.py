...
class WordList:
    ...
    def addWord(self, name, tagId, source, sourceid):
        try:
            print("adding word",name, source,sourceid)
            query = (
                "insert ignore  token (token, frage_id,source,source_id) values ('" + name+"','" + str(tagId) +"','"+source + "','" + sourceid + "')")
            print(query)
            self.cursor.execute(query)
            self.connection.commit()
        except mysql.connector.Error as err:
    ...
    def AddFoundKeywords(self, keyword_id, token_id):
            try:
                query = (
                    "insert ignore  keywordsFound (frage_id,token_id) values ('" + str(keyword_id) + "','" + str(token_id) + "')")
                # print(query)
                self.cursor.execute(query)
                self.connection.commit()
            except mysql.connector.Error as err:
